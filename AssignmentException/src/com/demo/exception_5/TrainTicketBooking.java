		...
		try {
			if (selection == 1) {
				...
				if (time >= 23 && time <= 1) {
					...
					throw new TicketBookingEception("BookingNotAllowedException ");
				...
				if (inputage > 5) {
					...
					if (number > 0 && number <= 6) {
						...
						throw new TicketBookingEception("normalTicketBookingException");
					}
				...
				if (d.getHours() >= 10 && d.getHours() <= 12) {
					...
					if (inputage > 5) {
						...
						if (number > 0 && number <= 4) {
							...
							throw new TicketBookingEception("booking limit exceed");
						}
					...
					throw new TicketBookingEception("TatkalTicketBookingException");
				}
